...
class TestApp(EWrapper, EClient):
    ...
    def tickPrice(self, reqId: TickerId , tickType, price: float, attrib):
        ...
        if tickType == 1:
        ...
        elif tickType == 4: #This is for LTP
            ...
            if(self.order_for_placing == True):
                ...
                if(self.open_inventory_position_at_giventime[c.symbol] == 0):
                    ...
                    Delta = self.Risk_aversion_Coefficient*Last_Price*Last_Price*self.YEARLY_Volatility_for_Lookback_Period*self.YEARLY_Volatility_for_Lookback_Period*self.TIME_HORIZON_in_Fractional_years
                    ...
                    B_p = round(B_p, 1)
                    #B_p = round(B_p, 2)
                    #ticks = round(B_p/0.03125)
                    #B_p = float(ticks * 3125)
                    #B_p = B_p / 100000
                    #B_p = round(B_p, 1)  
                    ...
                    A_p = round(A_p, 1)
                    #A_p = round(A_p, 2)
                    #ticks = round(A_p/0.03125)
                    #A_p = float(ticks * 3125)
                    #A_p = A_p / 100000
                    #A_p = round(A_p, 1) 
                ...
                #************************************3/Condition 2 START***********************************************# 
                    ...
                    self._send_lmt_order(self.contracts[reqId],"BUY",self.open_inventory_position_at_giventime[c.symbol],round(Last_Price - (Last_Price % GlobalMinTick), 1))
                    ...
                    self._send_lmt_order(self.contracts[reqId],"SELL",self.open_inventory_position_at_giventime[c.symbol],round(Last_Price - (Last_Price % GlobalMinTick), 1))
...
for i in range(len(contracts)):
    #Requesting market TickByTick Bid, Ask and Last Price Data
